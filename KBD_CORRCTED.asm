...
_RESET:
	...
	ldi	r16,0x10		; 16 -> r16
KBD_RESET_LOOP: // цикл 16 попыток сделать сброс клавиатуры
	rcall	LONG_WAIT_15		; вызов подпрограммы длинного цикла ожидания
	mov	KBD_BYTE,CONSTFF	; 0xFF->KBD_BYTE // Keyboard Command RESET
	rcall	KBD_SEND		; отправить команду сброса (0xFF)
	brcc	INIT_KBD		; если сбросилась то переходим на инициализацию
	dec	r16			; r16--
...
INIT_KBD:
	rcall	SMALL_WAIT
	rcall	KBD_READ		; вызов подпрограммы чтения с клавиатуры
...
.db "http://avray.ru",0x0D,0x0A,0x00
